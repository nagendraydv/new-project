        ...
        try:
            if False:#not validate.Request(api='companyGroup', request=input_dict):
                ...
                if val_error:
                    ...
                    join=Query.from_(kyc).join(compDocMap,how=JoinType.left).on(kyc.DOCUMENT_TYPE_ID==compDocMap.DOCUMENT_ID).select(kyc.DOCUMENT_TYPE_ID,kyc.DOCUMENT_TYPE,kyc.DOCUMENT_GROUP)
                    q1=join.where(compDocMap.COMPANY_ID==input_dict["data"]["companyID"])
                    db.pikastr(q1)
